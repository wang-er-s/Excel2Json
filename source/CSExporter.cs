...
namespace ExcelToJson
{
    ...
    static class CSExporter
    {
        ...
        public static string GenEnum(string excelPath)
        {
             // 加载Excel文件
             IExcelDataReader excelReader = Tools.CreateExcelReader(excelPath);
             // 数据检测
        ...
        {
            StringBuilder outCompileCs = new StringBuilder(@"
public class #class_nameFactory{public List<#class_name> list = new List<#class_name>();}");
            outCompileCs.AppendLine("public class #class_name{#fields}");
            ...
            {
                if (field.Type.Contains("Dictionary<"))
                {
                    sb.AppendLine("\t[MongoDB.Bson.Serialization.Attributes.BsonDictionaryOptions(MongoDB.Bson.Serialization.Options.DictionaryRepresentation.ArrayOfArrays)]");
                }
            ...
            foreach (FieldDef field in fieldList)
            {
                ...
                sb.AppendLine("\t[MongoDB.Bson.Serialization.Attributes.BsonElement]");
                if (field.Type.Contains("Dictionary<"))
                {
                    sb.AppendLine(
            ...
            if (!string.IsNullOrEmpty(Options.Default.ScriptPath))
            {
                ...
                template = template.Replace("#load_config_path",
                    Options.Default.ScriptLoadJsonPath.Replace("#json_name",
                        typeName + Options.Default.JsonFileExtension));
